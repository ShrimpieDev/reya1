...
## Avoiding merge conflicts next time
If you are collaborating through pull requests, use this flow every time before coding and before pushing:
...
If your repository uses `master`, replace `origin/main` with `origin/master`.

When rebase completes, run checks and push with lease:
...
Tips:
- Keep PRs small (single feature/fix) so fewer files overlap.
- Avoid resolving large JS conflicts in the GitHub web editor; resolve locally/Codespaces, run checks, then push.
- If conflicts occur, prefer taking the latest known-good file version from your branch commit, then re-run checks.
